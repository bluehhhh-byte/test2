...
async function loadRss() {
  try {
    ...
    el('rssStatus').textContent = 'RSS를 직접 불러오지 못해 보조 목록을 표시합니다.';
    el('rssList').innerHTML = (window.__RSS_ITEMS__ || []).slice(0, 5).map((item) => `<li><a href="${item.link}" target="_blank" rel="noreferrer">${item.title}</a><div><time>${item.date || ''}</time></div><p>${item.desc || ''}</p></li>`).join('');
  }
}
